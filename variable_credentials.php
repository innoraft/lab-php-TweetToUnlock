<?php
$hashtag= "saveearth";  // change your hastag quote
$tweet_req_to_unlock= 5;	// total tweets to unlock an item
$user_tweet_allowance= 5;	// total tweets that can be made by the user
$username = 'root';		
$password = '1234';
$servername = 'localhost';
$dbname= 'twitter';
$consumer_key = "nL5Rd0xqnAJ8JLz2M5Aje0Ium";
$consumer_secret = "CRaKId5hj2nUxtW8LAoeaBsadt4uqAX8I3qeHpOnsdpyRWeAFa";
$access_token = "2937099032-WPGL18SyWpMQ21KVV5qFBAf6C8uw7rd1hfZlbx2";
$access_token_secret = "a4YPQAIxpYreLFPsdlARIgRnHUpmqHuLl4pLaiFqnOVZd";
$admin_pass_default= "ayan1234";  //set name,email and password for default admin to login
$admin_name_default= "AYAN";
$admin_email_default= "ayan.majumder@innoraft.com";
$home= "http://ecotweet.sites.innoraft.com";
date_default_timezone_set('Asia/Kolkata');
?>